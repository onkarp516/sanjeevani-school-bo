...
import {
  ...
  getFeesMasterDetailsByStandard,
...
class StudentFeesPayment extends React.Component {
  constructor() {
    super();
  ...
  handleInputChange(e, index) {
    var items = this.state.feesMasterDetails;
    ...
    this.setState({
      feesMasterDetails: items,
    });
  }

  getTotalAmount = () => {
    let { feesMasterDetails } = this.state;
    let amt = 0;
    feesMasterDetails.map((v) => {
        ...
        if (res.responseStatus == 200) {
          ...
          // if (d.length > 0)
          {
            let Opt = d.map(function (values) {
              return {
                value: values.id,
                label: values.standardName,
                standMaxFee: values.maxFee,
              };
            });
  ...
  getInstallmentStructure = () => {
    ...
    console.log("initVal", this.state.initVal);

    let reqData = new FormData();
    // reqData.append("feesMasterId", feesMasterId);
    reqData.append("studentId", studentId.value);
    reqData.append("academicYearId", academicYearId.value);
    reqData.append("standardId", standardId.value);
    reqData.append("studentType", studentType.value);
    // reqData.append("studentGroup", studentGroup);
    reqData.append("branchId", branchId.value);
    // reqData.append(
    //   "installmentNo",
    ...
    if (payable != "") {
      ...
      // }
      getFeesMasterDetailsByStandard(reqData)
        .then((response) => {
          let res = response.data;
          if (res.responseStatus == 200) {
            this.setState({ feesMasterDetails: res.responseObject });
  ...
  submitCalculations = () => {
    let requestData = new FormData();
    let { initVal: values, feesMasterDetails } = this.state;
    requestData.append(
      "instituteId",
      authenticationService.currentUserValue.companyId
    );
    ...
    requestData.append(
      "paymentNo",
      values.paymentNo == "" ? 0 : values.paymentNo
    );
    ...
    // requestData.append(
    //   "transactionId",
    //   values.transactionId !== undefined ? values.transactionId : "0"
    // );
    ...
    let balance =
      values.outstanding -
      (parseInt(values.payable) + parseInt(values.discount));
    if (values.transactionId == 0) {
      requestData.append("balance", balance);
    } else {
      requestData.append("balance", balance);
    }
    requestData.append("payable", values.payable);
    requestData.append("discount", values.discount);
    requestData.append("totalFees", values.outstanding);
    requestData.append("outstanding", values.outstanding);
    requestData.append("row", JSON.stringify(feesMasterDetails));
        ...
        <Formik
          ...
          render={({
            ...
            >
                    ...
                    <Row className="">
                      <Col md="12" className="mb-2">
                        ...
                        <Row className="row-inside">
                              ...
                              <Select
                                ...
                                onChange={(v) => {
                                  ...
                                  if (v != null) {
                                    ...
                                    setFieldValue("outstanding", v.maxFee);
                          ...
                          {/* </Row>
                        <Row className="row-inside"> */}
                          {(values && parseInt(values.outstanding) > 0) ||
                            // &&
              ...
              {/* {JSON.stringify(parseInt(this.state.initVal.payable))}
              {/*  {JSON.stringify(this.state.opStandardList.standMaxFee)} */}
              {/* {JSON.stringify(values.standardId.standMaxFee)} */}
              {feesMasterDetails && feesMasterDetails.length > 0 && (
                            ...
                            return (
                              <>
                                <tr>
                                  ...
                                  <td>
                                    {/* {JSON.stringify(parseInt(this.state.initVal.payable))}
                                    {JSON.stringify(this.state.initVal.outstanding)} */}
                                    {values.standardId.standMaxFee ==
                                      this.state.initVal.payable ? (
                                      //parseInt(this.state.initVal.outstanding) == parseInt(this.state.initVal.payable)
                                      v.amount ? (
                                        v.amount
                                      ) : (
                                        0
...
export default connect(mapStateToProps, mapActionsToProps)(StudentFeesPayment);
